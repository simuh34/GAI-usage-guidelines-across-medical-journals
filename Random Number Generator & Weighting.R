...
##1.whole range journals 144
total_population <- 7174
group_size <- 100
sample_per_group <- 2
total_groups <- total_population %/% group_size
remaining_samples <- total_population %% group_size
full_group_samples <- (total_groups +1) * sample_per_group
weight_full_group <- ((group_size/total_population) / (sample_per_group/full_group_samples))

if (remaining_samples > 0) {
  weight_partial_group <- ((remaining_samples/total_population) / (sample_per_group/full_group_samples))  
} else {
...
sample_weights <- c(rep(weight_full_group, full_group_samples-2))

if (remaining_samples >= 2) {
  sample_weights <- c(sample_weights, rep(weight_partial_group, 2)) 
}
print(sample_weights)
write.csv(sample_weights,"D:\\R project\\GAL\\weight_144.csv")


##2.
# whole range journals 140
...
print(sample_weights)
write.csv(sample_weights,"D:\\R project\\GAL\\weight_140.csv")
